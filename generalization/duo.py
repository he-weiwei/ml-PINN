...
class PINN_model:
    def __init__(self, layers, X, q, X_uc, u_c, X_ac, a_c, X_kc, k_c):
        ...
        self.loss_f = tf.reduce_mean(tf.square(self.f_Q_q_pred[:, 0:1])) \
            + tf.reduce_mean(tf.square(self.f_M_Q_pred[:, 0:1])) \
            + tf.reduce_mean(tf.square(self.f_a_k_pred[:, 0:1])) \
            + tf.reduce_mean(tf.square(self.f_u_k_pred[:, 0:1]))
...
if __name__ == '__main__':
    ...
    layers = [[3]+3*[50]+[1], [3]+3*[50]+[1], [3]+3*[50]+[1], [3]+3*[50]+[1]]
    ...
    q = -5*np.random.rand(X_train.shape[0], X_train.shape[1])
    u = 0.2*np.random.rand(X_train.shape[0], X_train.shape[1]) - 0.1

    X_train = np.concatenate((X_train, q, u), 1)




    X_uc_0 = np.array([[0.0]]).repeat(N_f, 0)
    u_c_0 = np.zeros_like(X_uc_0)
    X_uc_0 = np.concatenate((X_uc_0, q, u), 1)
    X_uc_1 = np.array([[1.0]]).repeat(N_f, 0)
    u_c_1 = u
    X_uc_1 = np.concatenate((X_uc_1, q, u), 1)
    X_uc = np.concatenate((X_uc_0, X_uc_1), 0)
    u_c = np.concatenate((u_c_0, u_c_1), 0)
    
    
    X_ac = np.array([[0.0], [1.0]]).repeat(N_f/2, 0)
    a_c = np.array([[0.0], [0.0]]).repeat(N_f/2, 0)
    X_ac = np.concatenate((X_ac, q, u), 1)
    
    X_kc = np.array([[0.0], [1.0]]).repeat(N_f/2, 0)
    k_c = np.array([[0.0], [0.0]]).repeat(N_f/2, 0)
    X_kc = np.concatenate((X_kc, q, u), 1)
    ...
    q_inp, u_inp = -2.0, 0.01
    X_star_ = np.concatenate((X_star, q_inp * np.ones_like(X_star), u_inp * np.ones_like(X_star)), 1)
    Q_pred, k_pred, a_pred, u_pred = model.predict_Q(X_star_), model.predict_k(X_star_), model.predict_a(X_star_), model.predict_u(X_star_)
    ...
    plt.plot(X_star[::25, 0:1], u_pred[::25, :], 'o-',  label='Generalized PINN', linewidth=1.0, c='#cd5334')
    #plt.plot(X_train[:, 0:1], np.zeros(X_train[:, 0:1].shape), 'o', alpha=0.6, label='Nodes, num='+str(N_f), c='#FD841F')
    ...
    plt.title('q = %.1f,  u = %.2f'%(q_inp, u_inp))
    plt.show()
