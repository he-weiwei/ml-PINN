...
class PINN_model:
    def __init__(self, layers, X, X_b):
        ...
        self.a_b = np.ones((X_b[0].shape[0], 1))
        ...
        self.a_b_1_pred = self.net_u_(self.x_b_1_tf, self.y_b_1_tf)
        self.a_b_2_pred = self.net_u_(self.x_b_2_tf, self.y_b_2_tf)
        self.a_b_3_pred = self.net_u_(self.x_b_3_tf, self.y_b_3_tf)
        self.a_b_4_pred = self.net_u_(self.x_b_4_tf, self.y_b_4_tf)

        
        self.k_b_1_pred = self.net_k(self.x_b_1_tf, self.y_b_1_tf)
        self.k_b_2_pred = self.net_k(self.x_b_2_tf, self.y_b_2_tf)
        self.k_b_3_pred = self.net_k(self.x_b_3_tf, self.y_b_3_tf)
        self.k_b_4_pred = self.net_k(self.x_b_4_tf, self.y_b_4_tf)
        ...
        self.loss_b_a = 2*tf.reduce_mean(tf.square(self.a_b_1_pred[:, 1:2] + self.a_b)) \
            + 2*tf.reduce_mean(tf.square(self.a_b_2_pred[:, 1:2] - self.a_b)) \
            + 2*tf.reduce_mean(tf.square(self.a_b_3_pred[:, 0:1] + self.a_b)) \
            + 2*tf.reduce_mean(tf.square(self.a_b_4_pred[:, 0:1] - self.a_b)) \
        ...
        self.loss_c = 1e3*self.loss_b_u + self.loss_b_a
...
if __name__ == '__main__':
    ...
    X_b1_star, X_b2_star, X_b3_star, X_b4_star = make_data_b(n=21)
    X_b_star = [X_b1_star, X_b2_star, X_b3_star, X_b4_star]
    X_star = make_data_all(n=21)
    ...
    data = xlrd.open_workbook('shell_data.xls')
    table = data.sheets()[1]
    ...
    plt.subplot(1, 3, 1)
    plt.contourf(x, y, u_pred_, levels=np.linspace(-0.25, 0.01, 30), cmap=plt.get_cmap('Spectral'))
    ...
    plt.subplot(1, 3, 2)
    plt.contourf(x, y, u_fem_, levels=np.linspace(-0.25, 0.01, 30), cmap=plt.get_cmap('Spectral'))
    plt.colorbar(format='%.2f').set_label(label='u(x, y)', loc='center', size=15)
    plt.xlabel('x', size=15)
    plt.ylabel('y', size=15)
    ...
    plt.plot(x[0], u_fem_[5], '-', label='FEM, y=0.1', linewidth=3.0, zorder=1, c='#FD841F')
    plt.plot(x[0], u_pred_[5], '^-',  label='PINN, y=0.1', linewidth=1.0, markersize=5, c='#3A8891')
    ...
    plt.plot(x[0], u_fem_[:,5], '-', label='FEM, x=0.1', linewidth=3.0, zorder=1, c='#FD841F')
    plt.plot(x[0], u_pred_[:,5], '^-',  label='PINN, x=0.1', linewidth=1.0, markersize=5, c='#3A8891')
