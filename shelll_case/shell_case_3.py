...
class PINN_model:
    def __init__(self, layers, X, X_b):
        ...
        self.loss_b_a = 2*tf.reduce_mean(tf.square(self.a_b_1_pred[:, 1:2])) \
            + 2*tf.reduce_mean(tf.square(self.a_b_2_pred[:, 1:2])) \
            + 2*tf.reduce_mean(tf.square(self.a_b_3_pred[:, 0:1])) \
            + 2*tf.reduce_mean(tf.square(self.a_b_4_pred[:, 0:1])) \
...
if __name__ == '__main__':
    ...
    plt.subplot(1, 3, 1)
    plt.contourf(x, y, u_pred_, levels=np.linspace(-0.14, 0.004, 30), cmap=plt.get_cmap('Spectral'))
    ...
    plt.subplot(1, 3, 2)
    plt.contourf(x, y, u_fem_, levels=np.linspace(-0.14, 0.004, 30), cmap=plt.get_cmap('Spectral'))
    plt.colorbar(format='%.2f').set_label(label='u(x, y)', loc='center', size=15)
    plt.xlabel('x', size=15)
    plt.ylabel('y', size=15)
